...
init_scraper <- function(baseUrl, windows = FALSE){
...
# 81.0.4044.138,83.0.4103.39,84.0.4147.30
  ...
  while( notdone && attempt <= 2 ) { # Auto restart when fails
    ...
    try({
      ...
      for (i in 1:length(headers)) {
        if (grepl("??????", headers[i], fixed = TRUE)) {view_no <- strtoi(gsub(",","", gsub("??????:","", headers[i])))}
        else if (grepl("??????", headers[i], fixed = TRUE)) {reply_no <- strtoi(gsub(",","", gsub("??????:","", headers[i])))}
        else if (grepl("??????", headers[i], fixed = TRUE)) {following_no <- strtoi(gsub(",","", gsub("??????:","", headers[i])))}
      }
      ...
      for (postpg in 1:lastpg){
        ...
        while (notdone_postpg && attempt_postpg <= 2) {
          ...
          try({
            ...
            # Post date
            post_date <- strsplit(gsub("\n","", html %>% html_nodes(".postinfo .post-date") %>% html_text()), "?????????")
...
init_scraper_posts <- function(start=start, stop=stop) {
  ...
  tryCatch({
    ...
    setwd(outfilePath)
    init_scraper(baseUrl="https://news.discuss.com.hk/", windows = FALSE)
    launch_browser()
  })
...
init_scraper_posts(start = 1, stop = 2)
